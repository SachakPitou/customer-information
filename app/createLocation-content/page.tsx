...
export default function CreateLocation() {
  ...
  const [insertLocationId, setInsertedLocationId] = useState('');
  ...
  useEffect(() => {
    if (insertLocationId || error) {
      setIsModalOpen(true);
    }
  }, [insertLocationId, error]);
  ...
  const handleAddLocation = async (e) => {
    ...
    try {
      ...
      setInsertedLocationId(newLocationId);
      setLocationName('');
    } catch (error) {
  ...
  return (
    <div className="flex flex-col w-full items-center justify-center min-h-screen dark:bg-gray-200">
      <div className="font-raleway-black w-full max-w-4xl p-5">
        ...
        isOpen={isModalOpen}
        onClose={closeModal}
        title={insertLocationId ? 'Success' : 'Error'}
        content={
          <>
            {insertLocationId && (
              <p className="text-center text-green-700 mt-4">
                Location created successfully with ID: {insertLocationId}
              </p>
            )}
            {error && (
              <p className="text-center text-red-700 mt-4">Error creating location: {error}</p>
            )}
          </>
        }
      />
